...
func TestNormalizeElectraTariffsMultiWindow(t *testing.T) {
	...
	// 03:00 falls in the first window: the representative price must be
	// the price of the window covering the current time, not the cheapest
	// one across the whole day.
	at := time.Date(2026, 7, 16, 3, 0, 0, 0, time.UTC)
	tariffs := normalizeElectraTariffsAt(pricing, at)
	...
	if app.EnergyPriceCentsPerKWh == nil || *app.EnergyPriceCentsPerKWh != 35.0 {
		t.Errorf("app.EnergyPriceCentsPerKWh at 03:00 = %v, want 35.0 (the window covering 03:00)", app.EnergyPriceCentsPerKWh)
	}
	...
	// 12:00 falls in the second window instead.
	atNoon := time.Date(2026, 7, 16, 12, 0, 0, 0, time.UTC)
	tariffsNoon := normalizeElectraTariffsAt(pricing, atNoon)
	for _, tariff := range tariffsNoon {
		if tariff.Plan != "app" {
			continue
		}
		if tariff.EnergyPriceCentsPerKWh == nil || *tariff.EnergyPriceCentsPerKWh != 55.0 {
			t.Errorf("app.EnergyPriceCentsPerKWh at 12:00 = %v, want 55.0 (the window covering 12:00)", tariff.EnergyPriceCentsPerKWh)
		}
	}
}

func TestNormalizeElectraTariffsSessionCongestionPerWindow(t *testing.T) {
	pricing := map[string]any{
		"dc_combo": map[string]any{
			"currency": "EUR",
			"windows": []any{
				map[string]any{
					...
					"energy_price_cents_per_kwh": 35.0, "session_duration_price_cents_per_min": 1.0, "congestion_price_cents_per_min": 5.0,
				},
				map[string]any{
					"start_time": "07:00", "end_time": "23:59",
					"energy_price_cents_per_kwh": 55.0, "session_duration_price_cents_per_min": 2.0, "congestion_price_cents_per_min": 10.0,
				},
			},
		},
	}
	...
	// The 03:00 window's own session/congestion must be used, not whichever
	// window was processed last while building the slice.
	if app.SessionPriceCentsPerMin == nil || *app.SessionPriceCentsPerMin != 1.0 {
		t.Errorf("app.SessionPriceCentsPerMin at 03:00 = %v, want 1.0", app.SessionPriceCentsPerMin)
	}
	if app.CongestionPriceCentsPerMin == nil || *app.CongestionPriceCentsPerMin != 5.0 {
		t.Errorf("app.CongestionPriceCentsPerMin at 03:00 = %v, want 5.0", app.CongestionPriceCentsPerMin)
	}
	...
	if windows[0]["sessionPriceCentsPerMin"] != 1.0 || windows[0]["congestionPriceCentsPerMin"] != 5.0 {
		t.Errorf("windows[0] = %+v, want session=1.0 congestion=5.0", windows[0])
	}
	if windows[1]["sessionPriceCentsPerMin"] != 2.0 || windows[1]["congestionPriceCentsPerMin"] != 10.0 {
		t.Errorf("windows[1] = %+v, want session=2.0 congestion=10.0", windows[1])
	}
}
...
func TestTimeInWindow(t *testing.T) {
	cases := []struct {
		...
		{"03:00", "00:00", "07:00", true},
		{"07:00", "00:00", "07:00", false}, // half-open: end excluded
		{"12:00", "07:00", "23:59", true},
		{"23:00", "22:00", "06:00", true},  // wraps midnight
		{"05:00", "22:00", "06:00", true},  // wraps midnight
		{"10:00", "22:00", "06:00", false}, // wraps midnight, outside
	}
	for _, c := range cases {
		if got := timeInWindow(c.hm, c.start, c.end); got != c.want {
			t.Errorf("timeInWindow(%q, %q, %q) = %v, want %v", c.hm, c.start, c.end, got, c.want)
		}
	}
}
